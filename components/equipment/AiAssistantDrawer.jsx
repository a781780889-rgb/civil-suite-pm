...
import { callEquipmentAi } from '@/lib/equipmentApi.js';

/** يُستخدم بصيغتين: مضمّن في صفحة معدة (equipmentId محدَّد + quickActions)، أو مستقل (ملخص الأسطول فقط). */
export default function AiAssistantDrawer({ open, onClose, equipmentId, quickActions = [] }) {
  ...
  async function run(action, payload, label) {
    ...
    try {
      const res = await callEquipmentAi(action, payload);
  ...
  function handleAsk(e) {
    ...
    run('ask', { equipment_id: equipmentId, question }, `سؤال: ${question}`);
  }
      ...
      <div className="relative w-full max-w-md h-full bg-sheet border-r border-line flex flex-col shadow-xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-line shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-ink text-sm">مساعد إدارة المعدات الذكي</span>
          </div>
        ...
        {quickActions.length > 0 && (
          <div className="px-4 py-3 border-b border-line shrink-0 flex flex-wrap gap-2">
            {quickActions.map((qa) => (
              <button
                key={qa.label}
                onClick={() => run(qa.action, { equipment_id: equipmentId, ...qa.payload }, qa.label)}
                disabled={loading}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          <p className="text-[11px] text-ink-soft leading-relaxed bg-line/40 rounded-md px-3 py-2">
            كل ما يعرضه المساعد اقتراحات للمراجعة البشرية فقط، مبنية على بيانات هذه المعدة الفعلية، ولا يُنفَّذ أي إجراء تلقائياً.
          </p>
          {loading && (
        ...
        <form onSubmit={handleAsk} className="p-3 border-t border-line shrink-0 flex gap-2">
          <input
            ...
            placeholder="اسأل عن هذه المعدة..."
...
/** يعرض أي كائن JSON مُعاد من الذكاء الاصطناعي بشكل مقروء، أو نصاً عادياً مباشرة (رد ask نصّي لا JSON). */
function ResultRenderer({ data }) {
  if (typeof data !== 'object' || data === null) return <p className="text-sm text-ink whitespace-pre-wrap">{String(data)}</p>;
  return (
