// lib/hse/apiHelpers.js
// نفس نمط lib/equipment/apiHelpers.js (الموسّع من lib/pm/apiHelpers.js) تماماً - يعيد استخدام
// PmPermissionError وValidationError العامتين بدل تكرارهما لهذا القسم.
...
export function getActor(body, request) {
  const actor = body?.actor || request?.headers?.get?.('x-hse-actor') || null;
  const actor_role = body?.actor_role || request?.headers?.get?.('x-hse-actor-role') || 'hse_manager';
  return { actor, actor_role };
}

export function handleHseError(err) {
