...
export function listWorkOrdersPaged({ status, project_id, contract_id, partner_id, search, page = 1, pageSize = 30 } = {}) {
  ...
  const base = ` FROM biz_work_orders w${where}`;
  ...
  const rows = db
    .prepare(`SELECT w.*, c.name AS client_name, p.company_name AS partner_name${base} LEFT JOIN biz_clients c ON c.id=w.client_id LEFT JOIN biz_partners p ON p.id=w.partner_id ORDER BY w.updated_at DESC LIMIT @limit OFFSET @offset`)
