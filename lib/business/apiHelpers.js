// lib/business/apiHelpers.js
// نفس نمط lib/pm/apiHelpers.js تماماً - يعيد استخدام PmPermissionError/ValidationError نفسهما
// (هما فئتا خطأ عامتان بالفعل، وليستا خاصتين بالقسم الرابع تحديداً) بدل تكرارهما.
...
export function getActor(body, request) {
  const actor = body?.actor || request?.headers?.get?.('x-biz-actor') || null;
  const actor_role = body?.actor_role || request?.headers?.get?.('x-biz-actor-role') || 'business_manager';
  return { actor, actor_role };
}

export function handleBizError(err) {
